...
btnAnalyze.addEventListener('click', async () => {
    ...
    if (!textValue) {
        alert("Veuillez introduire un texte biblique avant de lancer l'analyse.");
        return;
    }
    ...
    try {
        ...
        const response = await fetch(API_URL, {
            ...
            body: JSON.stringify({ userBibleText: textValue })
        });
        ...
        if (data.success) {
            ...
            alert("Une erreur est survenue lors de l'analyse du serveur.");
        }
...
/**
 * Injecte dynamiquement les données reçues de Gemini et de MongoDB dans le HTML
 */
function displayResults(data) {
    // 1. Mettre à jour le bandeau de cadrage textuel (Genre et Méthode)
    document.getElementById('genreLittéraire').innerHTML = `Genre : ${data.genre_litteraire}`;
    document.getElementById('methodeAnalyse').innerHTML = data.methode_analyse_recommandee;
    console.log("Genre est", `Genre : ${data.genre_litteraire}`, "metheode est", data.methode_analyse_recommandee);

    // 2. Vider les anciens conteneurs de colonnes
    exegeseContainer.innerHTML = '';
    theologieContainer.innerHTML = '';
    proverbesContainer.innerHTML = '';

    // 3. Injecter les Mots-clés originaux analysés selon le genre littéraire
    ...
    // 4. Injecter les Clés Théologiques
    ...
    // 5. Injecter les pertinences canonique et luthérienne
    document.getElementById('canonLivre').textContent = data.pertinence_canonique.dans_le_livre;
    document.getElementById('canonGlobal').textContent = data.pertinence_canonique.dans_le_canon;
    document.getElementById('lutherDoctrinale').textContent = data.connexion_lutherienne.articulation_doctrinale;
    document.getElementById('lutherConfession').textContent = data.connexion_lutherienne.references_confessionnelles;

    // 6. Injecter les Proverbes Malagasy correspondants extraits de votre MongoDB
    ...
    // 6. Afficher l'ensemble des résultats avec une transition fluide
